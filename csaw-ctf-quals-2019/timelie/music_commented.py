...
def combine_lunar_frequencies(a, b):
    '''
    Thanks for this function L
    '''
    # convert a & b to psuedo sets
    a.removeRedundantPitchClasses()  # B_list
    b.removeRedundantPitchClasses()  # known chords
    for p in range(12):
...
def launder_vc_money(hexstr):
    '''
        converts the 3 digit hex number into a binary set of Chord's
        L
    '''
...
def begin_sales_presentation(list_of_chords, keystream):
    # add padding for launder_vc_money
    # len(keystream) % 3 has three possible output 0,1,2
    # mod can be 3, 2 or 1
    mod = 3 - len(keystream) % 3
    if mod != 0:                                        # big brain energy
        keystream = ('0'*mod ) + keystream              # pad no fool me
    if len(list_of_chords) < (len(keystream) / 3):
        raise ValueError("That's not long enough!")

    b_chords = []
    # convert the hex key stream into list of binary chords
    for i in range(0, len(keystream), 3):
        key_chord = launder_vc_money(keystream[i:i+3])
        b_chords.append(key_chord)

    # sets of list_of_chords - b_chords
    output= []
    for i in range(len(list_of_chords)):
        x = combine_lunar_frequencies(b_chords[i%len(b_chords)], s[i])
        output.append(x)
    return output
